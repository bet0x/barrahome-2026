...
The K8s deployment unit is the `DynamoGraphDeployment` (DGD) CRD (`v1beta1` as of v1.0.0, with config versioning and rolling worker updates). Service discovery runs via [etcd](https://etcd.io); KV events flow through [NATS](https://nats.io).
...
- **Dynamo Snapshot** — fast GPU worker recovery via CRIU and cuda-checkpoint; reduces cold-start time on pod restarts without full model reload.
- **Flash Indexer** — the KV block indexer used by the router hits 170M ops/sec in v1.0.0, a 42x improvement over the initial release.
...
<tr><td><b>Maturity</b></td><td>GA, enterprise supported</td><td>v1.0.0 GA, open source, Apache 2.0</td></tr>
