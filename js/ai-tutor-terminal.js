(function () {
    ...
    var API_BASE = window.BARRAHOME_OPENAI_API_BASE || "https://api.openai.com";
    var MODEL = window.BARRAHOME_OPENAI_MODEL || "gpt-4o-mini";
    var STORAGE_KEY = "barrahome_tutor_api_key";
    ...
    function createUI() {
        ...
        style.textContent = ""
            ...
            + ".ai-tutor-toolbar input,.ai-tutor-toolbar select{background:#d8dbe6;border:1px solid #6b6e7a;padding:2px 6px;font-family:monospace;font-size:12px;}"
        ...
        titlebar.innerHTML = "<span>Article Tutor Terminal</span><button type=\"button\" id=\"ai-tutor-close\" class=\"ai-tutor-btn\">X</button>";

        var toolbar = document.createElement("div");
        toolbar.className = "ai-tutor-toolbar";
        toolbar.innerHTML = ""
            + "<label for=\"ai-tutor-model\">model</label>"
            + "<input id=\"ai-tutor-model\" value=\"" + MODEL + "\" />"
            + "<label for=\"ai-tutor-key\">api_key</label>"
            + "<input id=\"ai-tutor-key\" type=\"password\" placeholder=\"sk-...\" />"
            + "<span>Hotkey: `</span>";
        ...
        status.textContent = "Ready. Using full article context in each request.";
        ...
        var keyInput = document.getElementById("ai-tutor-key");
        var modelInput = document.getElementById("ai-tutor-model");
        ...
        keyInput.value = window.BARRAHOME_OPENAI_API_KEY || sessionStorage.getItem(STORAGE_KEY) || "";
        ...
        function addLog(role, content) {
            ...
            row.textContent = (role === "user" ? "> " : "assistant> ") + content;
            log.appendChild(row);
        ...
        async function send() {
            ...
            var pageTitleEl = document.querySelector(".content h1, .content h2, .content h3");
            ...
            var payload = {
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are an expert tutor for technical blog articles. Only use the article context provided by the user. If missing info, say so clearly."
                    },
                    {
                        role: "user",
                        content:
        ...
        document.addEventListener("mousemove", function (e) {
            if (!dragging) return;
            var left = Math.max(8, Math.min(window.innerWidth - win.offsetWidth - 8, e.clientX - dx));
            var top = Math.max(8, Math.min(window.innerHeight - win.offsetHeight - 8, e.clientY - dy));
